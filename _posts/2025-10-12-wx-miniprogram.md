...
- [ ] 实现一个“三页流程”：`A(首页) → B(填写信息) → C(结果页)`  
      要求：在 C 页点击“返回首页”后，不能再从 A 返回到 B、C
- [ ] 在首页列表中点击某项进入详情页，返回后保持列表滚动位置不变
- [ ] 实现登录页：  
      - 未登录用户从“我的”tab 进入 → 跳到登录页  
      - 登录成功后 `reLaunch` 到“我的”tab
- [ ] 使用 `navigateBack({ delta: 2 })` 完成多级页面返回的实战
